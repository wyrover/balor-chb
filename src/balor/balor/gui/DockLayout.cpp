...
Size DockLayout::preferredSize() const {
	...
	Padding parentMargin(0); // ドッキングコントロ?ルがそれぞれの枠に占める大きさ。
	Size minSize = Size(0, 0); // ドッキングコントロ?ルの可変サイズ部分の最小値の合計。
	ChildInfo defaultInfo;
	for (auto i = _target->controlsBegin(); i; ++i) {
		if (!i->visibleExceptParent()) {
			continue;
	...
	for (auto i = _target->controlsBegin(); i; ++i) {
		...
		if (info->style == Style::none) {
			...
			preferredSizeForOther.width = std::max(preferredSizeForOther.width, bounds.right() + margin.right + parentMargin.right); // parentMargin を加えることで dockコントロ?ルと重ならないようにする
			preferredSizeForOther.height = std::max(preferredSizeForOther.height, bounds.bottom() + margin.bottom + parentMargin.bottom); // parentMargin を加えることで dockコントロ?ルと重ならないようにする
		}
	}
