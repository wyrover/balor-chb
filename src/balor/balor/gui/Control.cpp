...
#include <WindowsX.h>
#include <Uxtheme.h> // XP以?。SetWindowTheme/GetWindowTheme
...
inline ATOM getThisPointerPropAtom() {
	class ThisPointerPropAtomHolder {
		...
		ThisPointerPropAtomHolder() {
			...
			atom = AddAtomW(atomName); // ロ?カルアト?はアプリケ?ション終了時にすべて開放されるので後始末は考えなくて良い
		}
		~ThisPointerPropAtomHolder() {}
...
Control* Control::Handle::control() const {
	...
	// 残念ながら GWLP_USERDATA は XP の IME など、一部のアプリケ?ションで使われていて
...
Control::Procedure Control::Handle::procedure(Procedure value) {
	assert("Different thread" && !differentThread());
//#pragma warning(push)
//#pragma warning(disable : 4244) // '引数' : 'LONG_PTR' から 'LONG' への変換です。デ??が失われる可?性があります。
...
void Control::focus() {
	assert("focus invidible control" && visible()); // 非?示のコントロ?ルにフォ?カスを移す事ができるが無意味である。
	SetFocus(handle());
}
...
void Control::font(HFONT value) {
	...
	invalidate(); // Label や GroupBox 等一部コントロ?ルは更新されないので
}
...
Control* Control::getNextControl(Control& start, bool forward) const {
	...
	// start に this を指定した場合、自分の子供のなかから?ブオ???の最小（forward) あるいは最大のものを返す
	if (forward) {
		// 子コントロ?ルから探す
		auto i = ControlsIterator(*control);
		if (i) {
			Control* found = &(*i);
			while(++i) { // 子コントロ?ルの中で最小の tabIndex を持つものを探す
		...
		while (control != this) {
			...
			for (auto i = parent->controlsBegin(); i; ++i) { // 兄弟の中で次の?ブオ???を持つものを探す
				if (control != i) {
					if (control->tabIndex() <= i->tabIndex()) { // control よりも tabIndex が大きく
						if (!found || i->tabIndex() < found->tabIndex()) { // 以前見つけたものより tabIndex が小さい。（より control に近い）
							if (i->tabIndex() != control->tabIndex() || hitControl) { // control と同じ?ブオ???なら Z オ???が次のもの
								found = i;
							}
						}
					}
		...
		if (control != this) {
			...
			bool hitControl = false;
			Control* found = nullptr;
			for (auto i = parent->controlsLast(); i; --i) { // 兄弟の中で前の?ブオ???を持つものを探す
				if (control != i) {
					if (i->tabIndex() <= control->tabIndex()) { // control より tabIndex が小さく
						if (!found || found->tabIndex() < i->tabIndex()) { // 以前見つけたものより tabIndex が大きい。（より control に近い）
							if (i->tabIndex() != control->tabIndex() || hitControl) { // control と同じ?ブオ???なら Z オ???が前のもの
								found = i;
							}
						}
					}
		...
		while (i) { // 最も低い子孫の階層で最大の tabIndex を持つものを探す
			Control* found = &(*i);
			while(--i) { // 子コントロ?ルの中で最後の tabIndex を持つものを探す
...
Size Control::getPreferredSize(int width, int height) const {
	...
	if (i) {
		...
		for (; i; ++i) {
			...
			bounds.position(bounds.position() - displayRect.position()); // スクロ?ルを打ち消す。
...
Region Control::region() const {
	...
	if (GetWindowRgn(handle(), region) == ERROR) { // リ?ジョン領域が設定されていなければ作成しない（空領域のままだとウインドウに設定すると?示されなくなるので挙動がおかしくなる）
		region = Region();
	}
	return region;
}
	...
	switch (msg.message) {
		...
		case WM_CONTEXTMENU : {
			if (onPopupMenu()) {
				...
				if (clientRectangle().contains(position)) { // クライアント領域に限定する
					auto frame = findFrame();
					if (!frame || !frame->menuBarFocused()) { // メニュ?バ?の onRightClick イベントがうまくいかなくなるのでメニュ?バ?を?作中は?示しない。
		...
		case WM_HELP : {
			if (onHelpRequest()) {
				...
				if (info->iContextType == HELPINFO_WINDOW) { // メニュ?のヘルプなんて使い方知ってる人少ないだろう。
		...
		case WM_HSCROLL :
		case WM_VSCROLL : {
			if (!reflectMessage((HWND)msg.lparam, msg)) { // スクロ?ルバ?コントロ?ル（独立したコントロ?ルのほう）は親に直接メッセ?ジを送る。
				processMessageByDefault(msg);
			}
		} break;
		...
		case WM_MOUSELEAVE : {
			processMessageByDefault(msg);
			endMouseTracking();
			_dragBox = hugeBox; // ウインドウのはじっこで??ンを押してウインドウ外にでて戻ってくるとドラッグが発生するのを防ぐ
		} break;
		case WM_MOUSEMOVE : {
