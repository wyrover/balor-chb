 ...
 * 文字列を表す。
 *
 * Java や .NET の String と同様、Unicode のみを扱い文字列ポインタの内容の変更を許可しない。
 * std::wstring と相互に変換可能。ただし basic_string を引数に取るテンプレート関数には渡せないものが多いのでその場合は begin、end 関数を使う。
 * 関数引数に balor::locale::Locale を渡さない場合は常に序数処理を行う。（地域差を無視して文字コードを数値として比較する）
 * 特に operator < 等の演算子の処理では Locale を指定できないので全て序数処理になることに注意。
 * 序数処理では全角アルファベットに対する ignoreCase はサポートしない。
 * Convert::to<String> 関数で変換可能な型と String は + 演算子で連結することができる。
 *
 * Unicode 正規化の為の isNormalized, normalize 関数は Vista 以降か、IE7 以降のインストールされた XP か、または normaliz.dll をインストールした XP でのみサポートする。
 *
 * Vista 以降であれば ::FindNLSString 関数が使えるのだが balor は XP 以降のサポートなので
 * indexOf, lastIndexOf, startsWith 及び endsWith 関数は自力実装となっている。
 * API が対応していないのでサロゲートペアでかつ結合文字である文字コードを結合文字と判断できない。
 *
 * 内容を変更しないという仕様を生かして refer, literal 関数を使って文字列ポインタを参照だけすることができるが、ポインタ参照の危険性を十分に考慮する必要がある。
 *
 * 文字列クラスの自作は車輪の再発明であり、できればやりたくはないが
 * std::wstring 及び Boost.string algo ではロケールや Unicode 関連の処理を完全に無視しているので対応した文字列クラスを作成した。
 *
 * ※サロゲートペア、正規化、結合文字は Unicode の仕様
 *
 * <h3>・サンプルコード</h3>
 * <pre><code>
	String s(L"abc");
...
public:
	...
	struct CompareOptions {
		enum _enum {
			...
			ignoreKanaType = 0x00010000, /// ひらがなとカタカナを区別しない。
	...
	/// 欠けたサロゲートペアなどの正規化不能な文字を正規化しようとしたときに投げられる。
...
public:
	...
	/// 文字列ポインタを参照する String を作成する。String は内容の変更を許さないが、参照元が先に破壊されると危険。
