...
/**
 * Event を引数に取る返値の無い関数や関数オブジェクトをｴ�ﾀﾔ、追加できる関数オブジェクト。
 *
 * std::function<void (Event)> とだいたい同じだが .NET のデリゲ?トのように operator+= による複数の関数追加をサ??トする。
 * operator-= による関数の削除は関数オブジェクトの比較方?が無いため未実装で operator= で上書きするか pop 関数で末尾から削除するしかない。
 * std::function 同様、引数が変換可?であれば引数が違う?の関数も登?できる。（例えば継承関係のある参照?同士など）
 * ラ??式をｴ�ﾀﾔする場合は?イン?一個分までのキャプ?ャならばメモリ割り当ては発生しない。
 * 
 * <h3>・関数?イン?の比較に関する問題</h3>
 * 関数?イン?やメンバ関数?イン?の比較をする場合、DLL プロジェクトをまたいで使うと同じ関数でもアドレスが異なる場合がある。
 * SymGetSymFromAddr関数によるシン?ル名の取得は、必ず pdb フ?イルを添付しなければならないのであまり実用的ではない。
 * この問題は balor をス?ティックライブラリではなく DLL にしてしまえば少なくとも balor 内部では解決するが
 * ユ?ザ関数については依然として関数をDLL内で定?しない限り問題が発生する。
 */
#pragma warning(push)
#pragma warning(disable : 4521) // '::balor::Listener<Sender, Event>' : 複数のコピ? コンストラク??が指定されています。
#pragma warning(disable : 4522) // '::balor::Listener<Sender, Event>' : 複数のｴ�ﾀﾔ演算子が指定されています。
...
#pragma warning(pop)
	...
	Listener(Listener& value) { value.function()->clone(*function()); } // T&&版の呼び出し防?
	Listener(const Listener& value) { value.function()->clone(*function()); }
	Listener(Listener&& value) { value.function()->moveTo(*function()); }
	Listener(const Listener&& value) { value.function()->clone(*function()); } // T&&版の呼び出し防?
	...
	Listener& operator=(Listener& value) { // T&&版の呼び出し防?
		if (&value != this) {
			function()->~Function();
	...
	Listener& operator=(const Listener&& value) { // T&&版の呼び出し防?
		if (&value != this) {
			function()->~Function();
			value.function()->clone(*function());
		}
