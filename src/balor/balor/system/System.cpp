...
bool System::is64BitOs() {
	...
	GetNativeSystemInfo(&info); // XP以降ならこの関数はある
	bool result = false;
	switch (info.wProcessorArchitecture) {
		...
		default : assert(false); break; // 不明なＣＰＵアーキテクチャ
	}
	return result;
}
...
bool System::is64BitProcess() {
	...
	assert(process);
	verify(IsWow64Process(process, &result)); // XP以降ならこの関数はある
	return result == FALSE;
}
...
System::OsKind System::osKind() {
	...
	} else { // infoEx.dwPlatformId == VER_PLATFORM_WIN32s または新しいなにか
	}
...
bool System::visualStyleEnabled() {
	static int comCtl32Version = 0; // DLL ごとに実体を持ってもかまわないし処理が重複してもかまわない。
	if (!comCtl32Version) {
