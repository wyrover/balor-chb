...
#include <balor/system/windows.hpp> // IsBadWritePtr, IsBadReadPtrのassertの為だけに必要
...
int MemoryStream::read(void* buffer, int offset, int count) {
	...
	const int readCount = std::min(std::max(0,(int)( _last - _current)), count); // _current は skip によって _last より後ろに行くことができる
