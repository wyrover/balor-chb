...
public:
	...
	ScopeExitFunctionHolder(const ScopeExitFunctionHolder& value) : function(value.function) {
		static_assert(false, "RVO (Return Value Optimization) is indispensable!"); // 戻り値最適化されなかったら二回関数が呼ばれてしまうのでエラ?にする
	}
...
#endif
/// ?クロ定数も含めて文字列を連結する?クロ関数
#define BALOR_DETAIL_CAT_STRING(a, b) BALOR_DETAIL_CAT_STRING_IMPL(a, b)
#define BALOR_DETAIL_CAT_STRING_IMPL(a, b) a ## b


/**
 * 関数オブジェクトをデストラクト時に実行するオブジェクトを自動生成する?クロ関数。
 *
 * boost/scope_exit.hpp とだいたい同じ。いわゆる RAII イディオ?に使用する。
 * この恐ろしい?クロはプログラ?の scopeExit という単語を全て書き換えてしまうので決してヘッ?フ?イルに include してはならない。
