 ...
 * }
 * </code></pre>
 * 문제는 라인2로, 여기서는 (1)T의 메모리 할당 (2)T의 컨스트럭터、(3)instance 변수에 대입, 총 3번의 처리를 수행하는데,
 ...
 * 문서에 있듯이 환경에 의존하는 방법으로 회피하는 것은 가능하지만 인스턴스의 초기화를 해결해도 mutex의 초기화에 대해서는 제자리 걸음이 되어 해결되지 않는다. 또한 문제1도 여전히 남는다.
 *
 * <h3>●해결책:</h3>
 ...
 *  락으로 메모리 배리어를 포함하기 위한 'DLCP같은 방법이 불필요하게 된다.
 * DLL의 글로벌 변수는 프로세스에 Attach하기 전에 초기화되는 것이 보증되기 때문에,
 ...
 * 글로벌 변수 mutex를 사용하기 때문에 모든 형의 초기화 처리가 공통으로 락된다.
